# gpio-interrupt-test.py
# GPIO12에 입력이 들어오면 문장을 출력한다.
# 라이브러리 불러오기
import RPi.GPIO as GPIO
import time

# 스위치 눌렸을 때 콜백함수
def switchPressed(channel):
    print('channel %s pressed!!'%channel)

# GPIO setup
GPIO.setmode(GPIO.BCM)
GPIO.setup(26, GPIO.IN, pull_up_down=GPIO.PUD_UP) #ground 연결 -> PUD_UP
GPIO.setup(21, GPIO.IN, pull_up_down=GPIO.PUD_UP) #ground 연결 -> PUD_UP

# interrupt 선언
# GPIO.RISING  : LOW  -> HIGH 되는 순간
# GPIO.FALLING : HGIHG -> LOW 되는 순간
GPIO.add_event_detect(26, GPIO.RISING, callback=switchPressed, bouncetime=200)
GPIO.add_event_detect(21, GPIO.RISING, callback=switchPressed, bouncetime=200)

# 메인 루프
try:
    while 1:
        print(".")
        time.sleep(0.1)
finally:
    GPIO.cleanup()